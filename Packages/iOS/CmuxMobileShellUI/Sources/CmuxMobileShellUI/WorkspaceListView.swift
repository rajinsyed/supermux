...
struct WorkspaceListView: View {
    ...
    var groupedWorkspaces: [MobileWorkspacePreview] {
        ...
        // SUPERMUX:begin supermux-mobile-hide-project-workspaces (upstream reads `workspaces` directly; supermuxShownProjectIDs is empty under an active filter, so hiding stays inert while filtering)
        return supermuxFlatWorkspaces.filter {
            currentFilter.matches($0, parsedMachines: parsedMachines)
        }
        // SUPERMUX:end supermux-mobile-hide-project-workspaces
    }
    ...
    var body: some View {
        ...
        // SUPERMUX:begin supermux-mobile-hide-search (marker only — see SUPERMUX-TOUCHPOINTS.md #110)
        // Upstream moved `.searchable(text: $searchText)` out of this view into
        // `WorkspaceListSearchHost` (pre-iOS 26) and `MobilePrimaryTabScaffold`
        // (iOS 26+ search Tab); `searchText` is now injected by that host rather
        // than `@State` here, so the query is live again instead of always "".
        // Nothing is left to remove in this file. The fork's "no workspace-list
        // search bar" decision must be re-applied at the new host if still wanted.
