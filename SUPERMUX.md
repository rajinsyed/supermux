...
8. **Start Claude in a new worktree (prompt-first).** From a project row (Mac hover ✦ button /
   context menu; phone swipe, long-press menu, inline nested row, detail header) type the task
   instead of naming anything: the workspace title and branch are derived from the prompt (one AI
   call when the gateway key is set, an offline heuristic otherwise), the worktree is created, and
   the workspace opens with its terminal already running the chosen **Claude command** with the
   ...
   picker scoped to the selected model. Last model/effort is remembered per command.
...
| Start Claude in a new worktree (prompt-first, per-command model catalog) | ✅ | `Packages/SupermuxKit/Sources/SupermuxKit/Agent/` (`SupermuxAgentLauncherSettings`, `SupermuxAgentModelCatalog` + `SupermuxAgentCommandProbePlan`, `SupermuxPromptNaming`, `SupermuxAgentLaunchCommand`, `SupermuxAgentWorktreeLauncher` — the one shared path), `AI/SupermuxAIWorktreeNamer`; Mac UI `UI/SupermuxAgentWorktreeSheet(+Chips)` mounted from the project row (`SupermuxProjectRowActions.newAgentWorktree`); wired in `SupermuxComposition.agentLaunch`. Catalogs cache in the harness `SupermuxHarnessModelCatalogStore` under `/supermux-agent-command/<cmd>` pseudo-paths |
...
| 20 | Start Claude in a new worktree | ✅ on iOS | `SupermuxAgentWorktreeSheet` (MobileUI) over `mobile.supermux.agent.options` / `agent.start`, gated on `supermux.agent_launch.v1`; store `SupermuxMobileAgentLaunchStore` (MobileKit). Entry points: project-row swipe (first revealed), long-press menu, inline nested "Start Claude in New Worktree" row, and the detail screen's Worktrees header ✦ — all through `SupermuxProjectsSectionModel.requestNewAgentWorktree`. Commands, catalogs, naming, git, and the terminal launch stay Mac-side (the phone cannot edit the command list; do that in the Mac sheet) |
