...
| 496 | `Packages/macOS/CmuxControlSocket/Sources/CmuxControlSocket/Wire/ControlCommandExecutionPolicy.swift` | `supermux-local-socket-mobile-methods` | Classifies every `mobile.supermux.*` method onto the socket-worker lane (never main-thread callable). Without this the methods classify `.mainActor`, whose synchronous legacy switch lacks the case, and the local control socket answers `method_not_found` — leaving local automation only the standalone-marked `workspace.create`, whose workspaces can never nest under a project |
| 497 | `Sources/TerminalController.swift` | `supermux-local-socket-mobile-methods` | Worker-lane async bridge (180s, `v2AsyncResultCall`) routing local-socket `mobile.supermux.*` requests into the fork's existing `v2MobileSupermuxDispatch`, so local automation (supermux-mcp) creates worktree workspaces through `SupermuxTabManagerOpener` with the project association that nests them in the sidebar. Mirrors the adjacent `mobile.task.models.list` bridge; re-apply by copying that shape with the prefix match and 180s timeout |
| 498 | `Packages/macOS/CmuxControlSocket/Tests/CmuxControlSocketTests/ControlCommandExecutionPolicyTests.swift` | `supermux-local-socket-mobile-methods-test` | Pins `mobile.supermux.*` methods to `.socketWorker(mainThreadCallable: false)` |
