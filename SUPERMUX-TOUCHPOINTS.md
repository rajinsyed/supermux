...
| 338 | `Sources/ContentView.swift` | `sidebar-coffee-mode-button` | In `SidebarFooterButtons`, the `shows(.help)` branch mounts the fork's `SupermuxCoffeeModeButton()` (`Sources/Supermux/SupermuxCoffeeModeButton.swift`, pbxproj ids `50BE0001…0105`/`…0106` under #3) **immediately after** the analytics button of #146b and before upstream's untouched `SidebarHelpMenuButton(onSendFeedback:)` — a purely additive one-line insert. A mug toggle holding IOKit keep-awake assertions (SupermuxKit `Power/`) so long-running agents are not cut off by system sleep. Deliberately NOT `pmset disablesleep`: that needs root and persists `SleepDisabled` across reboot, so a crash would leave the Mac permanently unable to sleep; assertions need no privileges and `powerd` releases them when the process exits, dies, or is SIGKILLed |
