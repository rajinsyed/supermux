...
- Numbering: the highest number in use is **296**. Numbers **4, 19, 52, 89, 106, 121, 142** are unused;
...
| 268 | `Sources/Sidebar/AppKitList/Cells/SidebarWorkspaceRowCellView.swift` | `supermux-unread-badge-capsule` | `layoutContent` measures the badge through the shared AppKit helper instead of assuming a `16 * fontScale` square; the leading advance and the trailing reservation both use the measured width so a wide badge can neither overlap the title nor be clipped. **Badge height is floored at the old square**, so a non-wrapping row's height is unchanged. **The trailing reservation is deliberately keyed on whether the row HAS a badge, not on whether it is currently visible**: hover hides the trailing badge behind the close button, and keying off visibility would widen a wrapping title on hover, unwrapping a line — while the table holds the height it cached at `isPointerHovering: false`. Reserving unconditionally keeps row height hover-independent, which is what that cache assumes |
...
| 286 | `Packages/iOS/CmuxMobileShell/Sources/CmuxMobileShell/MobileShellComposite.swift` | `supermux-mobile-selection-sync` | Integration fences store one serialized optimistic selection pipeline and one effective focused-panel value; send workspace plus the selected terminal/browser/Simulator when navigation or the picker changes; cancel pending work with other remote operations; and route list application through the fork reconciliation policy only when `supermux.selection_sync.v1` or `.v2` is advertised. Older/upstream Macs retain the prior phone-local behavior |
| 287 | `Packages/iOS/CmuxMobileShell/Sources/CmuxMobileShell/MobileShellComposite+SupermuxSelectionSync.swift` | `supermux-mobile-selection-sync` | **Whole-file fork addition inside an upstream package.** Serializes rapid phone selections, uses v1 workspace/terminal RPCs for version skew and v2 `mobile.supermux.panel.select` for every panel kind, holds an optimistic request id against stale list frames, refreshes from the authoritative Mac after success, and rolls back explicitly after failure |
| 288 | `Packages/iOS/CmuxMobileShell/Tests/CmuxMobileShellTests/SupermuxSelectionSyncTests.swift` | `supermux-mobile-selection-sync` | **Whole-file fork test inside an upstream package.** Pins Mac→phone workspace, terminal, browser, and Simulator focus; stale browser-frame suppression while a phone mutation is pending; v1 terminal fallback; and the upstream-host fallback |
...
| 307 | `Packages/iOS/CmuxMobileShellUI/Sources/CmuxMobileShellUI/WorkspaceDetailView.swift` | `supermux-mobile-selection-sync` | Routes phone browser/Simulator picker selections through the shared shell mutation, and observes Mac-authoritative focus plus late browser discovery to mount the matching streamed surface without echoing an RPC back to the Mac. Terminal selection closes streamed chrome as before |
...
the build is Release, untagged, production-auth, fixed to `com.supermux.ios`, automatically signed
under the configured team with local personal-team entitlements, signature-verified, installed, and
launched. Neither test may sign, install, or launch a real app. Keep them in a cheap preflight job
