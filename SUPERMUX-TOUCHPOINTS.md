...
- Numbering: the highest number in use is **337**. Numbers **4, 19, 52, 89, 106, 121, 142** are unused;
...
| 338 | `Sources/ContentView.swift` | `sidebar-coffee-mode-button` | In `SidebarFooterButtons`, the `shows(.help)` branch mounts the fork's `SupermuxCoffeeModeButton()` (`Sources/Supermux/SupermuxCoffeeModeButton.swift`, pbxproj ids `50BE0001…0105`/`…0106` under #3) **immediately after** the analytics button of #146b and before upstream's untouched `SidebarHelpMenuButton(onSendFeedback:)` — a purely additive one-line insert. A mug toggle holding one public IOKit `PreventUserIdleSystemSleep` assertion (SupermuxKit `Power/`) so long-running agents are not cut off by idle system sleep while the display may turn off normally. Deliberately NOT `pmset disablesleep`: that needs root and persists `SleepDisabled` across reboot, so a crash would leave the Mac permanently unable to sleep; the app-scoped assertion needs no privileges and `powerd` releases it when the process exits, dies, or is SIGKILLed |
| 339 | `Sources/AppDelegate.swift` | `coffee-mode-restore` | One line in `applicationDidFinishLaunching`: `SupermuxComposition.restoreCoffeeMode()`. Load-bearing, not cosmetic — `coffeeModeModel` is a Swift `static let` (lazy) whose only other reference is the #338 button, and `SidebarFooterPresentationPolicy` hides the footer entirely in `minimal` presentation mode. Without this call, a Mac left with Coffee Mode on relaunches into that mode, never constructs the model, re-acquires nothing, and **silently sleeps** while the user believes their agents are protected |
...
The Coffee Mode sidebar toggle (touchpoint #338) adds file reference `50BE0001…0105` and build file
`50BE0001…0106` for `SupermuxCoffeeModeButton.swift`, wired in the same four places. Its
package-owned `Power/` sources are discovered automatically by SwiftPM and need no pbxproj entry.
