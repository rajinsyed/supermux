...
- Numbering: the highest number in use is **214**. Numbers **4, 19, 52, 89, 106, 121, 142** are unused;
...
| 213 | `Packages/iOS/CmuxMobileTerminal/Sources/CmuxMobileTerminal/GhosttySurfaceView.swift` | `ios-terminal-host-keyboard-sync` | **Fixes "keyboard covers the terminal" (upstream regression in `ba47b1dc0d`).** Upstream moved the keyboard layout guide onto `GhosttySurfaceHostView`, but `keyboardHeight` is refreshed only in the surface's own `layoutSubviews` — and the surface's frame never changes when the HOST's guide moves, so UIKit guarantees no surface relayout after the guide seats its new target; `keyboardHeight` stays 0 and the grid keeps full height under the keyboard. Two fenced blocks in `advanceBottomDockTransition` (the display-link pass): re-sample `synchronizeKeyboardGeometryFromLayoutGuide()` every frame and add `keyboardGeometryChanged` to the early-out guard so a changed target re-derives the viewport without a layout pass. Offer upstream. Re-apply: capture the sync result at the top of `advanceBottomDockTransition`, OR it into the transition guard |
