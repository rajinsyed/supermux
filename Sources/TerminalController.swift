...
class TerminalController {
    ...
    private func closeWorkspace(_ tabId: String) -> String {
        ...
        v2MainSync {
            if let tab = tabManager.tabs.first(where: { $0.id == uuid }) {
                ...
                // Upstream's closeWorkspaceNonInteractively closes the whole
                // WINDOW for a window's last workspace; supermux keeps the
                // window open as an empty home. Route through
                // closeWorkspace(allowEmptyingWindow:) and reply OK only if
                // the workspace actually left `tabs`.
    ...
    func v2MobileTerminalCreate(params: [String: Any]) -> V2CallResult {
        ...
        let createdTabID = workspace.surfaceIdFromPanelId(terminal.id)
        if v2Bool(params, "focus") == true,
           let focusError = v2SupermuxCreatedPanelFocusError(
               workspaceID: workspace.id,
               panelID: terminal.id
           ) {
            let closed = closeSurfaceRecordingHistory(
                in: workspace,
                surfaceId: terminal.id,
                force: true
            )
            if !closed, let createdTabID {
                _ = workspace.requestCloseTab(createdTabID, force: true)
            }
            return focusError
        }
