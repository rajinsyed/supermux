...
class TabManager: ObservableObject {
    ...
    // SUPERMUX:begin keep-window-on-last-close
    // Intentionally orphaned in the fork: upstream's call sites (the
    // last-workspace and batch/anchor close paths) are replaced by
    // keep-window-on-last-close, which routes through
    // closeWorkspace(allowEmptyingWindow:) instead of closing the window.
    // Upstream's no-op seam is kept verbatim so merges to it apply cleanly.
    // SUPERMUX:end keep-window-on-last-close
