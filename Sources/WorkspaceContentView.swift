...
struct WorkspaceContentView: View {
    ...
    var body: some View {
        ...
        // Render the piggycode-style terminal presets bar above the splits.
        // The content is upstream's canvas-vs-bonsplit Group; supermux only
        // wraps it with the always-mounted presets bar (upstream applies
        // `WorkspaceContentMinimalModeSafeAreaModifier` directly to the Group;
        // here it wraps the VStack instead). Minimal mode stays chrome-free:
        // the MOUNT hides the bar by reading the presentation mode itself, so
        // this body never reads it — a mode toggle re-evaluates neither this
        // view nor the canvas/bonsplit subtree, which upstream's
        // WorkspaceContentViewVisibilityTests probe asserts.
