...
final class CmuxFeatureFlags {
    ...
    private func applyRemoteFlagValues(_ values: [String: Bool]) {
        ...
        for definition in Self.allFlags {
            // SUPERMUX:begin appkit-sidebar-default-off
            // (upstream: `if let value = values[definition.key] {`) Upstream
            // moved production remote ingestion off `applyLoadedFlags` (now
            // reached only from tests) onto this control-plane loader path, so
            // the fork filter has to run here or a rollout `true` still pins the
            // AppKit sidebar on and blocks the Debug override. Filtering to
            // `nil` falls into upstream's `else`, which also evicts the cached
            // value from `defaults`. See `supermuxIngestibleRemoteValue`.
            if let value = Self.supermuxIngestibleRemoteValue(
    ...
    func applyLoadedFlags() {
        ...
        for definition in Self.allFlags {
            // SUPERMUX:begin appkit-sidebar-default-off
            // (upstream: `if let value = Self.coerceBoolFlagValue(remoteFlagValueProvider(definition.key)) {`)
            // Route the PostHog-SDK ingestion path through the same fork filter
            // as the control-plane loader. Filtering to `nil` falls into
            // upstream's `else if`, which evicts a cached `true`.
            // See `supermuxIngestibleRemoteValue`.
            if let value = Self.supermuxIngestibleRemoteValue(
                Self.coerceBoolFlagValue(remoteFlagValueProvider(definition.key)),
                for: definition.key
            ) {
            // SUPERMUX:end appkit-sidebar-default-off
                remoteValuesByKey[definition.key] = value
    ...
    // SUPERMUX:begin appkit-sidebar-default-off
    /// Filters a remote (PostHog SDK or control-plane) value before it is
    /// ingested into `remoteValuesByKey`.
    ///
    /// Ignores upstream's remote *rollout* (`true`) for the AppKit sidebar
    /// experiment: a remote `true` outranks both the fork's flipped default and
    /// the user's local override (`setOverride` refuses to shadow a remote
    /// value), and `appKitWorkspaceScrollArea` would then render
    /// `SidebarWorkspaceTableView` directly, bypassing the SwiftUI list that
    /// hosts every supermux sidebar feature. A remote `false` is upstream's
    /// kill-switch direction and still ingests, so a Debug opt-in cannot
    /// outlive an upstream emergency disable. The Debug local override remains
    /// the only way to opt in on the fork.
    ///
    /// Every site that writes `remoteValuesByKey` routes through here — the
    /// init cache seeding, `applyRemoteFlagValues` (the production
    /// control-plane path) and `applyLoadedFlags` — so there is one gate
    /// instead of one guard per ingestion path. Returning `nil` also lets each
    /// caller's existing eviction branch clear a `true` cached in
    /// `UserDefaults` by an earlier build.
    private static func supermuxIngestibleRemoteValue(
        _ value: Bool?,
        for key: String
    ) -> Bool? {
        guard key == appKitSidebarListFlag.key else { return value }
