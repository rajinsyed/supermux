...
struct ContentView: View {
    ...
    private func commandPaletteCommandContributions() -> [CommandPaletteCommandContribution] {
        ...
        // SUPERMUX:begin supermux-claude-harness-palette
        contributions.append(.supermuxNewClaudeHarness)
        // SUPERMUX:end supermux-claude-harness-palette
        contributions.append(
            CommandPaletteCommandContribution(
    ...
    private func registerCommandPaletteHandlers(_ registry: inout CommandPaletteHandlerRegistry) {
        ...
        // SUPERMUX:begin supermux-claude-harness-palette
        registry.registerSupermuxClaudeHarness(tabManager: tabManager)
        // SUPERMUX:end supermux-claude-harness-palette
...
struct VerticalTabsSidebar: View, Equatable {
    ...
    // Measured height of the SupermuxProjectsMount section; subtracted from the
    // viewport height handed to SidebarRowsFillLayout below so the empty-area
    ...
    private func legacyWorkspaceScrollArea(
        ...
        return GeometryReader { viewport in
            ...
            .onPreferenceChange(SupermuxProjectsSectionHeightPreferenceKey.self) { height in
                ...
                // since the rows' min-height fill is floored to a whole point.
                // Only dedupe shrink jitter (0.5pt) — an over-tall stored height
    ...
    private func workspaceScrollContent(
        ...
        let content = workspaceRows(
            ...
            // SUPERMUX:begin sidebar-projects-empty-area
            // Subtract the Projects section (mounted above in the same scroll
            // content) from the viewport handed to the rows' min-height fill,
            // so projects + rows + filler stay one viewport (no #3241 phantom
            // scroller); genuine overflow still scrolls (remainder clamps to 0).
        ...
        // The scrollable workspace list starts with the sticky Projects
        // section; the drop-target reader and drag overlays stay attached to
        // the rows subview so collected row frames keep their coordinates.
        return VStack(spacing: 0) {
    ...
    private func selectWorkspaceRow(
        ...
        if isShift, let anchorIndex = shiftAnchorIndex {
            ...
            let projectHiddenIds = supermuxProjectHiddenWorkspaceIds()
            // SUPERMUX:end sidebar-hide-project-workspaces
            let visibleRangeIds = tabManager.tabs[lower...upper].compactMap { candidate -> UUID? in
                // SUPERMUX:begin sidebar-hide-project-workspaces
                if projectHiddenIds.contains(candidate.id) {
                    return nil
                }
                // SUPERMUX:end sidebar-hide-project-workspaces
    ...
    /// event handlers and row-action closures only — never read from a row
    /// `body`, so the snapshot-boundary contract is untouched.
    private func supermuxProjectHiddenWorkspaceIds() -> Set<UUID> {
    ...
    private func makeWorkspaceRowActionFactory() -> SidebarWorkspaceRowActionFactory {
        ...
        return SidebarWorkspaceRowActions(
            ...
            closeOtherTargets: { targetIds in
                ...
                // SUPERMUX:begin sidebar-hide-project-workspaces
                // Same visible-rows-only scoping as closeTargetsBelow/Above:
                // "other" means the other rows the user can see in the flat
                // list, never the project-hidden workspaces rendered nested in
                // the Projects section.
                // (upstream: `keepIds.contains($0.id) ? nil : $0.id`)
                let projectHiddenIds = supermuxProjectHiddenWorkspaceIds()
                let idsToClose = tabManager.tabs.compactMap {
                    keepIds.contains($0.id) || projectHiddenIds.contains($0.id) ? nil : $0.id
                }
                // SUPERMUX:end sidebar-hide-project-workspaces
            ...
            closeTargetsBelow: {
                ...
                // project-hidden workspaces render nested in the Projects
                // section, so sweeping them here would destroy workspaces the
                // user can't see. Collapsed-group members deliberately stay
                // included (upstream semantics: their anchor row represents
                // them).
                let hiddenBelow = supermuxProjectHiddenWorkspaceIds()
                closeWorkspaceRows(
                    tabManager.tabs.suffix(from: anchorIndex + 1)
                        .map(\.id)
                        .filter { !hiddenBelow.contains($0) },
                    allowPinned: true
                )
                // SUPERMUX:end sidebar-hide-project-workspaces
            },
            closeTargetsAbove: {
                ...
                // SUPERMUX:begin sidebar-hide-project-workspaces
                // Same visible-rows-only scoping as closeTargetsBelow.
                let hiddenAbove = supermuxProjectHiddenWorkspaceIds()
                closeWorkspaceRows(
                    tabManager.tabs.prefix(upTo: anchorIndex)
                        .map(\.id)
                        .filter { !hiddenAbove.contains($0) },
                    allowPinned: true
                )
                // SUPERMUX:end sidebar-hide-project-workspaces
            },
...
struct TabItemView: View, Equatable {
    ...
    var body: some View {
        ...
        let rowView = VStack(alignment: .leading, spacing: 2) {
        // SUPERMUX:end sidebar-unified-row-style
            HStack(alignment: .sidebarTitleFirstLineCenter, spacing: titleRowSpacing) {
                ...
                if isEditing {
                    ...
                    Text(displayedTitle)
                        // SUPERMUX:begin sidebar-unified-row-style
                        // Title size matches the nested project-workspace rows
                        // (11.5·scale; upstream: 12.5).
