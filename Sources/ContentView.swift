...
struct VerticalTabsSidebar: View {
    ...
    private func workspaceScrollContent(
        ...
        VStack(spacing: 0) {
            ...
            // content) from the viewport handed to the fill layout, so
            // projects + rows + filler stay one viewport (no #3241 phantom
            // scroller); genuine overflow still scrolls (remainder clamps to 0).
            SidebarRowsFillLayout(viewportHeight: max(0, minHeight - supermuxProjectsSectionHeight)) {
            // SUPERMUX:end sidebar-projects-empty-area
