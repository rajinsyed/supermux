...
def main() -> int:
    ...
    # SUPERMUX:begin fix-resolved-policy-path-deps
    # Same unsatisfiable case as the per-package loop below: the workspace
    ...
    # real `-resolvePackageDependencies`). Only demand a diff when the workspace's
    # own remote closure actually moved.
    if ios_workspace_dependencies_changed:
        current_ios_remote_calls: set[str] = set()
        for root in current_ios_workspace_roots:
            current_ios_remote_calls |= closure_remote_dependency_calls(root, graph)
    ...
    for root, manifest in sorted(cmux_manifests.items()):
        ...
        # resolved (e.g. a fork package gaining a dep on an upstream package
        # that both already reach) leaves `swift package resolve` with nothing
        # to write here — the pins and the originHash are byte-identical — so
        # demanding a diff is unsatisfiable. The root whose OWN closure grew is
        # still caught: it is added to `changed_dependency_roots` above and its
        # own lockfile is required by the loop's first pass.
