...
while [ "$attempt" -le "$max_attempts" ]; do
  ...
  # (upstream: no trailing EXCLUDED_SOURCE_FILE_NAMES build setting above)
  # ibtoold deterministically crashes rendering the fork's Icon Composer
  # AppIcon*.icon files on some CI VMs, so headless CI builds skip them
  # entirely — the app icon is cosmetic here.
